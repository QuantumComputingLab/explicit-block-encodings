function circuit = div2(n,targets,controls,controlStates)
%DIV2   Divide by 2 circuit.
%   circuit = DIV2(n) constructs the n-qubit quantum circuit that
%   performs the division by 2 on the computational basis states.
%   The first qubit is used as ancilla and should be initialized by |0>.
%
%   circuit = DIV2(n,targets,controls) constructs the n-qubit quantum
%   circuit that performs the division by 2 on the target qubits in
%   TARGETS controlled by the qubits in CONTROLS.
%
%   circuit = DIV2(n,targets,controls,controlStates) constructs the
%   n-qubit quantum circuit that performs the division by 2 on the
%   target qubits in TARGETS conrtolled by the qubits in CONTROLS
%   with control states in CONTROLSTATES.
%
